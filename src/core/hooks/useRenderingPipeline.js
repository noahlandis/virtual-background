...
function useRenderingPipeline(sourcePlayback, backgroundConfig, segmentationConfig, bodyPix, tflite) {
    ...
    var _b = useState(0), fps = _b[0], setFps = _b[1];
    var _c = useState([]), durations = _c[0], setDurations = _c[1];
    useEffect(function () {
        var targetTimerTimeoutMs = 1000 / segmentationConfig.targetFps;
        var previousTime = 0;
        var beginTime = 0;
        var eventCount = 0;
        var frameCount = 0;
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            ...
                            beginFrame();
                            return [4, newPipeline.render()];
                        case 1:
                            _a.sent();
                            endFrame();
                            renderTimeoutId = timerWorker.setTimeout(render, Math.max(0, targetTimerTimeoutMs - (performance.now() - startTime)));
                            return [2];
                    }
                });
            });
